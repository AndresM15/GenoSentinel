...
public class GatewayController {
    ...
    // CAMBIO 1: Recibir 'byte[]' en lugar de 'Object' en AMBOS métodos (Genomics y Clinical)
    @RequestMapping(value = "/genomics/**", method = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE})
    public ResponseEntity<?> proxyGenomics(HttpServletRequest request, @RequestBody(required = false) byte[] body) {
    ...
    public ResponseEntity<?> proxyClinical(HttpServletRequest request, @RequestBody(required = false) byte[] body) {
        String requestPath = request.getRequestURI();
    ...
    // --- LÓGICA CORREGIDA ---
    private ResponseEntity<?> forwardRequest(URI uri, HttpServletRequest request, byte[] body) {
        try {
            ...
            while (headerNames.hasMoreElements()) {
                ...
                // CAMBIO 2: FILTRAR HEADERS PROBLEMÁTICOS
                // No copiamos la longitud porque puede cambiar al reenviar.
                // RestTemplate calculará la nueva longitud automáticamente.
            ...
            e.printStackTrace(); // Imprimir error en consola para debug
